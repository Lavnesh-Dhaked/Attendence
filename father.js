...
function parseCSV(data) {
  ...
  return students.map((line) => {
    const [studentName, fatherName] = line.split(","); // Split by comma
    return { studentName, fatherName }; // Return an object
  });
}
...
function displayStudents(students) {
  ...
  students.forEach((student, index) => {
    ...
    // Create the student name span with index and father's name
    const studentName = document.createElement("span");
    studentName.className = "student-name";
    studentName.textContent = `${index + 1}. ${student.studentName} (Father: ${
      student.fatherName
    })`;
    ...
    const leaveButton = createAttendanceButton("Leave", student.studentName);
    const absentButton = createAttendanceButton("Absent", student.studentName);
...
document.getElementById("downloadButton").addEventListener("click", () => {
  ...
  doc.text("Father's Name", 90, yOffset);
  ...
  students.forEach((student, index) => {
    const studentName = student
      ...
      .textContent.split(" (Father: ")[0];
    const fatherName = student
      .querySelector(".student-name")
      .textContent.split(" (Father: ")[1]
      .replace(")", "");
    ...
    doc.text(fatherName, 90, yOffset);
